...
public class ClientesController {
	...
	/* Método na API para gerar um serviço de exclusão de cliente */

	@DeleteMapping("excluir/{idCliente}") // Quando passa chaves, está passando a referência da URL.
	public String delete(@PathVariable("idCliente") UUID idCliente) {
		// @PathVariable é uma anotação para passar a variável através da url, no caso
		// aqui, idCliente.
		try {
	...
	public List<ConsultarClienteDto> getAll() {
		...
		try {
			...
			// percorrendo cada cliente da lista obtida do banco de dados
			for (Cliente cliente : lista) { //Para cada lista de cliente (entidade))
				//preenchendo a lista de clientes no DTO
				...
				clientes.add(dto); //Adicionando cada clienteDTO(dto) na lista DTO(clientes)
			}
